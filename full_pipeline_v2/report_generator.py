"""
Phase 3: Patient Report Generator (V2)
========================================
V2 changes:
  - Priority alerts section for critical signals
  - NOT APPLICABLE handling for gated branches
  - Disease category and progression in report header
  - Violence risk in report
"""
...
def format_report_text(
    ...
    lines.append(sep)
    lines.append("  CLINICAL PREPAREDNESS REPORT (V2)")
    ...
    # V2: Priority Alerts (if critical signals found)
    if critical_signals and critical_signals.get("signals_found"):
        lines.append("")
        lines.append("  ⚠️  CRITICAL SAFETY ALERTS")
        lines.append("-" * 40)
        for alert in critical_signals.get("priority_alerts", []):
            lines.append(f"  {alert}")
        lines.append("")

    # Report priority alerts from consistency checker
    if report.get("priority_alerts"):
        if not (critical_signals and critical_signals.get("signals_found")):
            lines.append("")
            lines.append("  ⚠️  PRIORITY ALERTS")
            lines.append("-" * 40)
