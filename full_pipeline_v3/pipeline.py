"""
Full Inference Pipeline V3: Orchestrator
==========================================
Unified pipeline that chains all phases with V3 improvements:
...
V3 additions over V2:
  - Context enricher injects housing/trauma/caregiver/weight/functional/prognosis
    into silver_labels so compact_factors() passes them to the scenario generator
  - Negation-aware signal scanner reduces false positive alerts
  - Duplicate alert deduplication
  - ICD code display fix
  - Severity inference when model returns "unknown"
  - Plausibility resolution from rationale text
...
def run_pipeline(
    ...
    """
    Run the full V3 pipeline on a single clinical note.
    ...
    log.info(f"Pipeline V3 complete in {duration:.1f}s")
    return result
...
def run_batch(
    ...
    """Process a JSONL file of clinical notes through the full V3 pipeline."""
    ...
    results_file = out_path / f"pipeline_v3_results_{timestamp}.jsonl"
    reports_dir = out_path / f"reports_v3_{timestamp}"
...
def run_single(
    ...
    """Run the full V3 pipeline on a single note."""
    multi_gpu = "," in gpu_ids
    load_adapters = not base_only
    ...
    log.info(f"Running V3 pipeline for subject={subject_id}, hadm={hadm_id} ...")
    ...
    json_file = out_path / f"result_v3_{subject_id}_{hadm_id}.json"
    ...
    if result.get("status") == "success":
        ...
        report_file = out_path / f"report_v3_{subject_id}_{hadm_id}.txt"
