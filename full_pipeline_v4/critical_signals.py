"""
Critical Signal Scanner (V3: with negation awareness)
======================================================
...
V3 improvements:
  - Negation awareness: "denies self-injury", "no violence" are downgraded
    to informational rather than treated as positive alerts
  - Deduplication of alerts before output
"""
...
# ============================================================================
# NEGATION DETECTION (V3)
# ============================================================================

# Words that negate a signal when they appear within N chars before the match
NEGATION_WORDS = [
    "denies", "denied", "deny", "denying",
    "no ", "not ", "none", "never",
    "negative", "without",
    "rules out", "ruled out",
    "has no", "no history of", "no evidence of",
    "absence of", "absent",
    "low risk of", "no active",
    "no known", "declines",
]

# Window (chars) to look back for negation before the match
NEGATION_LOOKBACK = 40


def _is_negated(note_text_lower: str, match_start: int) -> bool:
    """
    Check if a signal match is negated by context words appearing
    just before it in the text.
    """
    lookback_start = max(0, match_start - NEGATION_LOOKBACK)
    preceding = note_text_lower[lookback_start:match_start]
...
def scan_critical_signals(note_text: str) -> Dict:
    ...
    results = {
        ...
        "alert_summary": "",
        "priority_alerts": [],
        "negated_signals": [],  # V3: track negated signals separately
    }
    ...
    for category, patterns in CRITICAL_SIGNAL_PATTERNS.items():
        ...
        for pattern in patterns:
            for match in re.finditer(pattern, note_lower):
                ...
                # V3: Check for negation
                negated = _is_negated(note_lower, match.start())
        ...
        if category_matches:
            ...
            # V3: Separate positive and negated signals
            positive_matches = [m for m in unique_matches if not m["negated"]]
            negated_matches = [m for m in unique_matches if m["negated"]]

            if positive_matches:
                all_matches[category] = positive_matches
            if negated_matches:
                results["negated_signals"].extend(
                    f"[NEGATED] {category}: '{m['matched_text']}' — {m['context']}"
