"""
Full Inference Pipeline V4: Orchestrator
==========================================
Unified pipeline that chains all phases with V4 improvements:

  Phase 1:    Silver Label Extraction (finetuned model)
  Phase 1.5:  Post-Extraction Normalization (ICD validation, Dx reconciliation)
  Phase 1.55: Context Enrichment (housing, trauma, caregiver, weight, function, prognosis)
  Phase 1.6:  Critical Signal Scanning (rule-based, negation-aware)
  Phase 1.7:  Branch Gating (evidence-based conditional logic)
  Phase 2:    Tree-of-Thoughts Scenario Generation (context-enriched, gated)
  Phase 3:    Patient Preparedness Report
  Phase 3.5:  Final Consistency Check (risk tier, severity, plausibility)
  Phase 4:    Counterfactual Evidence Attribution (XAI)
  Phase 4.5:  DOCX Report Generation (color-highlighted document)

V4 additions over V3:
  - Evidence extractor maps scenario outputs back to input note spans
  - Branch-colored span highlighting (Red/Orange/Blue/Yellow)
  - DOCX report with highlighted clinical note + full preparedness report
"""
...
from .evidence_attribution import extract_evidence_attribution
...
def run_pipeline(
    ...
    """
    Run the full V4 pipeline on a single clinical note.

    Pipeline flow:
      1.    Extract silver labels (or use pre-provided)
      1.5   Normalize: ICD validation, Dx reconciliation, disease classification
      1.55  Context enrichment: housing, trauma, caregiver, weight, function, prognosis
      1.6   Scan critical signals: suicide/violence detection (negation-aware)
      1.7   Evaluate branch gates: skip irrelevant scenario branches
      2.    Generate scenarios (context-enriched, only for active branches)
      3.    Generate report
      3.5   Final consistency check (severity inference, plausibility resolution)
      4.    Counterfactual Evidence Attribution (XAI)
      4.5   DOCX report generation (optional)
    ...
    # ================================================================
    # PHASE 1.55: Context Enrichment (NEW in V3)
    # ================================================================
    log.info("Phase 1.55: Running context enrichment ...")
    silver_labels = enrich_context(
        silver_labels=silver_labels,
        note_text=cleaned_text,
    )

    # ================================================================
    # PHASE 1.6: Critical Signal Scanning (V3: negation-aware)
    # ================================================================
    ...
    # ================================================================
    # PHASE 4: Counterfactual Evidence Attribution (XAI) — NEW in V4
    # ================================================================
    log.info("Phase 4: Running Counterfactual Evidence Attribution (XAI) ...")
    evidence_result = extract_evidence_attribution(
        note_text=cleaned_text,
        scenarios_result=scenarios_result,
        silver_labels=silver_labels,
        critical_signals=critical_signals,
    ...
    result = {
        ...
        "phase1_55_enrichment_log": silver_labels.get("_enrichment_log", []),
        ...
        "phase4_evidence_attribution": evidence_result,
    }

    log.info(f"Pipeline V4 complete in {duration:.1f}s")
    return result
...
def run_batch(
    ...
    """Process a JSONL file of clinical notes through the full V4 pipeline."""
    ...
    results_file = out_path / f"pipeline_v4_results_{timestamp}.jsonl"
    reports_dir = out_path / f"reports_v4_{timestamp}"
        ...
        for line_num, line in enumerate(fin):
            ...
            try:
                ...
                if result.get("status") == "success":
                    ...
                    # Phase 4.5: DOCX report with evidence highlights
                    evidence = result.get("phase4_evidence_attribution", {})
                    cleaned_note = clean_report_text(note_text)
                    docx_path = str(reports_dir / f"report_{subject_id}_{hadm_id}.docx")
...
def run_single(
    ...
    """Run the full V4 pipeline on a single note."""
    multi_gpu = "," in gpu_ids
    load_adapters = not base_only
    ...
    log.info(f"Running V4 pipeline for subject={subject_id}, hadm={hadm_id} ...")
    ...
    json_file = out_path / f"result_v4_{subject_id}_{hadm_id}.json"
    ...
    if result.get("status") == "success":
        ...
        report_file = out_path / f"report_v4_{subject_id}_{hadm_id}.txt"
        ...
        evidence = result.get("phase4_evidence_attribution", {})
        cleaned_note = clean_report_text(note_text)
        docx_path = str(out_path / f"report_v4_{subject_id}_{hadm_id}.docx")
        ...
        # Phase 4.5b: PDF report with evidence highlights
        pdf_path = str(out_path / f"report_v4_{subject_id}_{hadm_id}.pdf")
