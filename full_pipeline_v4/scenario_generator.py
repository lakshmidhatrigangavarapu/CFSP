"""
Phase 2: Tree-of-Thoughts (ToT) Scenario Generation (V2)
==========================================================
V2 changes:
  - Conditional branch gating: skips branches with no evidence
  - Evidence binding: model must cite specific data points
  - Disease-aware prompts: neurodegenerative vs psychiatric vs medical
  - Gated-out branches produce "NOT APPLICABLE" entries instead of
...
# ============================================================================
# SYSTEM PROMPT — Scenario Generation (matches V1 proven prompt)
# ============================================================================
...
# ============================================================================
# STEP 3 — Scenario Narrative (V2: with evidence binding)
# ============================================================================
...
def _run_branch(
    ...
    # V2: Check if triggers are empty (model found no evidence)
    if isinstance(triggers, list) and len(triggers) == 0:
        log.info(f"  Branch {branch['id']}: Model found NO triggering factors. "
                 f"Marking as low-relevance.")
        return _build_empty_scenario(branch, "Model found no triggering factors in patient data")
...
def _build_empty_scenario(branch: dict, reason: str) -> dict:
    """Build a NOT APPLICABLE scenario entry for a gated-out branch."""
    return {
        "scenario_id": branch["id"],
        "branch_type": branch["type"],
        "branch_label": branch["label"],
        ...
        "causal_chain": [],
        "scenario": {
            "narrative": f"NOT APPLICABLE: {reason}",
            "warning_signs": [],
            "crisis_endpoint": "N/A — pathway not applicable for this patient",
            "preparedness_actions": [],
            "plausibility": "not_applicable",
            "plausibility_rationale": reason,
            "mental_health_relevance": "not_applicable",
            "mh_relevance_note": reason,
        },
    }


# ============================================================================
# FULL ToT GENERATION (V2: with branch gating)
# ============================================================================
...
def generate_scenarios(
    ...
    gated_branches: Optional[List[dict]] = None,
) -> dict:
    """
    Run the Tree-of-Thoughts scenario generation pipeline.

    V2: Accepts pre-evaluated branch gates. Skips gated-out branches
    with a NOT APPLICABLE marker.
    """
    ...
    if gated_branches is None:
        branches_to_run = BRANCHES[:num_branches]
        gated_branches = [dict(b, gated=False, gate_reason="No gating applied") for b in branches_to_run]

    scenarios = []
    active_count = 0
    gated_count = 0

    for branch in gated_branches[:num_branches]:
        if branch.get("gated"):
            # Branch gated out — produce NOT APPLICABLE entry
            log.info(f"  Branch {branch['id']} ({branch['type']}): GATED OUT — {branch.get('gate_reason', '?')}")
            scenarios.append(_build_empty_scenario(branch, branch.get("gate_reason", "Gated out by evidence check")))
            gated_count += 1
        else:
            # Branch active — run full scenario generation
            result = _run_branch(
                branch=branch,
    ...
    log.info(f"Phase 2: {active_count} branches ran, {gated_count} gated out")

    return {
        "phase": "2_tree_of_thoughts_scenario_generation",
        "method": "tree_of_thoughts_v2",
        "num_branches": num_branches,
        "active_branches": active_count,
        "gated_branches": gated_count,
        ...
        "patient_factors_summary": {
            ...
            "disease_category": silver_labels.get("disease_category", "unknown"),
            "progression": silver_labels.get("progression", "unknown"),
            ...
            "violence_risk": silver_labels.get("violence_risk_indicators", {}).get("aggression_present", False),
