#!/usr/bin/env python3
"""
Full Pipeline V4 CLI Runner
=============================
Usage:
  python -m full_pipeline_v4 --mode single --input one_sample.jsonl --gpu 0
  python -m full_pipeline_v4 --mode single --input input-output/input.txt --gpu 0
  python -m full_pipeline_v4 --mode batch --input one_sample_test10.jsonl --gpu 0,1
"""
...
# Set CUDA_VISIBLE_DEVICES BEFORE torch is imported (via pipeline -> model_loader)
_gpu_arg = "0"
...
from full_pipeline_v4.config import (
    BASE_MODEL_ID,
    CHECKPOINT_PATH,
    NUM_BRANCHES,
    OUTPUT_DIR,
)
from full_pipeline_v4.pipeline import run_single, run_batch
...
def main():
    ...
    log = logging.getLogger("full_pipeline_v4")
    log.info("=" * 60)
    log.info("  Full Inference Pipeline V4")
    ...
    log.info("  V4 Features: all V3 features + XAI evidence attribution + DOCX report")
    log.info("=" * 60)
