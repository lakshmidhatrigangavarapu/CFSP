...
from full_pipeline_v4.docx_generator import generate_docx_report
...
def run_pipeline_action(
    ...
    while thread.is_alive():
        yield "", "", "", "", "", console.get_text(), None, "⏳ Pipeline running..."
        time.sleep(0.3)
    ...
    # --- Phase 4.5: Generate DOCX report ---
    docx_file = None
    try:
        ...
        docx_path = str(out_dir / f"report_v4_{subject_id}_{hadm_id}.docx")
        ...
        if docx_file:
            log.info(f"DOCX report saved → {docx_file}")
    except Exception as docx_err:
        log.warning(f"DOCX generation failed: {docx_err}")

    yield report_txt, silver_json, scenarios_json, evidence_json, full_json, console.get_text(), docx_file, status


def build_ui(default_gpu: str = "0"):
    with gr.Blocks(title="Clinical Pipeline V4 Tester") as demo:

        gr.Markdown("# 🏥 Clinical Pipeline V4 Tester")
        gr.Markdown(
            "V4 improvements: all V3 features + Counterfactual Evidence Attribution (XAI) "
            "with branch-colored highlighting + DOCX report generation."
        )
        ...
        with gr.Row(equal_height=False):
            with gr.Column(scale=1):
                ...
                run_btn = gr.Button("🚀 Run Pipeline V4", variant="primary", size="lg")
            ...
            with gr.Column(scale=2):
                ...
                with gr.Tabs():
                    with gr.Tab("📄 Report"):
                        report_out = gr.Textbox(label="Clinical Preparedness Report (V4)", lines=40, interactive=False)
                    ...
                    with gr.Tab("🔍 XAI Evidence"):
                        evidence_out = gr.Code(label="Counterfactual Evidence Attribution", language="json", lines=40, interactive=False)
            ...
            with gr.Column(scale=1):
                ...
                console_out = gr.Textbox(
                    label="Token Generation & Pipeline Logs",
                    lines=40,
                    interactive=False,
                )
                gr.Markdown("### 📥 Download")
                docx_out = gr.File(label="DOCX Report (with evidence highlights)")

        run_btn.click(
            fn=run_pipeline_action,
            inputs=[input_mode, text_input, json_input, random_note, use_labels_chk, branches_slider],
            outputs=[report_out, labels_out, scenarios_out, evidence_out, full_out, console_out, docx_out, run_status],
        )
...
def main():
    parser = argparse.ArgumentParser(description="Clinical Pipeline V4 Gradio UI")
