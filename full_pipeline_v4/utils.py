...
def compact_factors(extracted: dict) -> str:
    ...
    # V3: Enriched context from context_enricher
    ctx = extracted.get("enriched_context") or {}
    if ctx:
        lines.append("")
        lines.append("--- Additional Context (from clinical note) ---")

        housing = ctx.get("housing_crisis")
        if housing and housing.get("detected"):
            details = ", ".join(housing.get("details", []))
            lines.append(f"Housing Crisis: {details}")
            evidence = housing.get("evidence", "")
            if evidence:
                lines.append(f"  Evidence: {evidence[:200]}")

        caregiver = ctx.get("caregiver_loss")
        if caregiver and caregiver.get("detected"):
            details = ", ".join(caregiver.get("details", []))
            lines.append(f"Caregiver/Support Loss: {details}")
            evidence = caregiver.get("evidence", "")
            if evidence:
                lines.append(f"  Evidence: {evidence[:200]}")

        trauma = ctx.get("trauma_history")
        if trauma and trauma.get("detected"):
            details = ", ".join(trauma.get("details", []))
            lines.append(f"Trauma History: {details}")

        weight = ctx.get("weight_change")
        if weight and weight.get("detected"):
            details = ", ".join(weight.get("details", []))
            amount = weight.get("amount", "")
            lines.append(f"Weight Change: {details}" + (f" ({amount})" if amount else ""))

        functional = ctx.get("functional_decline")
        if functional and functional.get("detected"):
            details = ", ".join(functional.get("details", []))
            duration = functional.get("homebound_duration", "")
            lines.append(f"Functional Decline: {details}" + (f" (homebound {duration})" if duration else ""))

        prognosis = ctx.get("clinician_prognosis")
        if prognosis and prognosis.get("detected"):
            details = ", ".join(prognosis.get("details", []))
            statement = prognosis.get("statement", "")
            lines.append(f"Clinician Prognosis: {details}")
            if statement:
                lines.append(f"  Statement: {statement[:250]}")

        legal = ctx.get("legal_status")
        if legal and legal.get("detected"):
            details = ", ".join(legal.get("details", []))
            lines.append(f"Legal Status: {details}")
